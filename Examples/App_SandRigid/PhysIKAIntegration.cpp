...
public:
    ...
    std::shared_ptr<VPE::PhysIKACar> GetCar(uint64_t car_handle)  //car_handle是啥？小车个数
    {
        ...
        {  //我懂啦，一个函数想调用另一个函数的对象，就把这个对象设置为静态，然后调用时带上原来这个类的域就好啦
...
inline std::shared_ptr<SandSimulationRegion> SandSimulationRegion::Impl::Init(const SandSimulationRegionCreateInfo& info)  //info这一系列参数都传进去了吗？没有，甚至那俩函数功能都没有合并进去！
{
    ...
    {                                       //每次循环都应该针对m_car[u]
        ...
        m_car[u]->wheelupDirection    = PhysIKA::Vector3f(0, 0.25, 0 /*car_cache[u].wheels[0].translation.x, car_cache[u].wheels[0].translation.y, car_cache[u].wheels[0].translation.z*/);                                  //轮子在z轴方向上和底盘的相对位置，原来是(0, 1, 0)(0, 0.5, 0)//用注释里面的数据就会导致车轮的轴设置错误，深刻原因未知
