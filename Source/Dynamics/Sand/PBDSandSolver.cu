...
/**
	...
	*/
__device__ inline double _PBD_weight2D(const Vector3d& p0, const Vector3d& p1, SpikyKernel2D<double> kern, double smoothLen)
{
    Vector3d v = p0 - p1;
    v[1]       = 0.0;
...
/**
	...
	*/
__device__ inline Vector3d _PBD_weightGrad2D(const Vector3d& p0, const Vector3d& p1, SpikyKernel2D<double> kern, double smoothLen)
{
    Vector3d v = p0 - p1;
    v[1]       = 0.0;
    double r   = v.norm();
    ...
    double                damp      = 1.0)
{
    ...
    if (pId >= posArr.size() || particleType[pId] != ParticleType::SAND)
        return;
    ...
    for (int ne = 0; ne < nbSize; ne++)
    {
        ...
        if (particleType[j] != ParticleType::SAND && particleType[j] != ParticleType::BOUNDARY)
            continue;
        rawgrad += _PBD_weightGrad2D(pos_i, posArr[j], kern, smoothingLength) * massArr[j];
...
{
    // Integrate velocity.
    cuExecute(m_particlePos.size(), PBD_integrateVelocity, m_particlePos, m_particleVel, m_particleType, dt);
    return true;
}
...
PBDSandSolver::PBDSandSolver()
    : m_smoothingLength(0.05), m_neighbors(1), m_position(1)
{
...
void PBDSandSolver::updateUserParticle(DeviceArray<Vector3f>& userPoints)
{
    ...
    cuExecute(pos.size(), PBD_updateUserParticle, userPoints, pos, m_particleMass, m_particleRho2D, m_land, m_staticHeight, m_particleType, m_neighbors.getValue(), m_kernel, m_smoothingLength.getValue(), m_rho0);
}

void PBDSandSolver::setParticles(Vector3d* pos, Vector3d* vel, double* mass, ParticleType* particleType, int num, double rho0, double m0, double smoothLen, double mu, double h0)
{
    m_mu                         = mu;
    m_rho0                       = rho0;
    ...
    double                rho0)
{
    ...
    if (pIdx >= height.Nx() || pIdy >= height.Ny())
        return;
    ...
    for (int gi = 0; gi < 9; ++gi)
    {
        ...
        for (int i = 0; i < num; ++i)
        {
            ...
            if (particleType[nbId] != ParticleType::SAND)
                continue;
...
void PBDSandSolver::computeSandHeight()
{
    ...
    cuExecute3D(totalsize, PBD_computeSandHeight, m_height, m_staticHeight, m_particlePos, m_particleMass, m_particleType,
...
void PBDSandSolver::computeSandStaticHeight()
{
    ...
    uint3 totalsize = { m_SandInfo.nx, m_SandInfo.ny, 1 };
    cuExecute3D(totalsize, PBD_computeSandStaticHeight, m_staticHeight, m_height, m_particlePos, m_particleMass, m_particleType,
                m_neighborQuery->getHash(),  //m_gridParticleHash,
                m_kernel,
                m_smoothingLength.getValue(),
                m_rho0);
    ...
    double                     smoothingLength)
{
    ...
    rho2d = _PBD_weight2D(pos_i, pos_i, kern, smoothingLength) * massArr[pId];
    for (int ne = 0; ne < nbSize; ne++)
    {
        int j = neighbors.getElement(pId, ne);
        ...
        if (particleType[j] != ParticleType::SAND && particleType[j] != ParticleType::BOUNDARY)
            continue;
        rho2d += _PBD_weight2D(pos_i, posArr[j], kern, smoothingLength) * massArr[j];
    }
    parRho2D[pId] = rho2d;
}
...
void PBDSandSolver::_updateRawParticleRho()
{
    ...
    cuExecute(m_particlePos.size(), PBD_updateRawParticleRho, m_particleRho2D, m_particlePos, m_particleMass, m_particleType, m_neighbors.getValue(), m_kernel, m_smoothingLength.getValue());
}
...
void PBDSandSolver::_updateParticlePos3D()
{
    ...
    cuExecute(m_particlePos.size(), PBD_updateParticlePos3D, m_particlePos3D, m_particlePos, m_particleRho2D, m_land, m_rho0);
    ...
    double dt)
{
    int pIdx = threadIdx.x + (blockIdx.x * blockDim.x);
    int pIdy = threadIdx.y + (blockIdx.y * blockDim.y);
    if (pIdx >= staticHeight.Nx() || pIdy >= staticHeight.Ny())
        return;
...
{
    uint3 totalsize = { m_SandInfo.nx, m_SandInfo.ny, 1 };
    cuExecute2D(totalsize, _PBD_UpdateStaticHeightChange, m_dStaticHeight, m_staticHeight, m_height, m_land, 1.0, atan(m_mu), m_h0, dt);
}
    ...
    double                       dl)
{
    int pIdx = threadIdx.x + (blockIdx.x * blockDim.x);
    int pIdy = threadIdx.y + (blockIdx.y * blockDim.y);
    if (pIdx >= staticHeight.Nx() || 2 * pIdy >= staticHeight.Ny())
        return;
    ...
    DeviceDArray<Vector3d>       particlePos)
{
    ...
    if (!gridParticle.inRange(gid.x, gid.y))
        return;
...
void PBDSandSolver::_generateAndEliminateParticle(double dt)
{
    ...
    //// Generate particles.
    uint3 totalsize = { m_SandInfo.nx, m_SandInfo.ny, 1 };
    cuExecute2D(totalsize, _PBD_generateParticle, m_particlePos, m_particleVel, m_particleMass, m_particleType, m_genEliCount.begin(),
    ...
    cuExecute(m_particlePos.size(), _PBD_findOneParticleOnGrid, gridParticle, gridParticleValid, m_particleType, m_particlePos);

    // Particle elimination.
    //cudaMemset(m_genEliCount.begin(), 0, sizeof(int));
    cuExecute2D(totalsize, _PBD_eliminateParticle, m_particleVel, m_particleType,
