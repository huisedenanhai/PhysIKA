...
namespace PhysIKA {
HeightFieldRigidDetectionModule::~HeightFieldRigidDetectionModule()
{
}

void HeightFieldRigidDetectionModule::setSize(int nx, int ny)
{
    m_nx = nx;
    m_ny = ny;
...
void HeightFieldRigidDetectionModule::setSize(int nx, int ny, float dx, float dy)
{
    this->setSize(nx, ny);
    m_heightField.setSpace(dx, dy);
}

__global__ void hfrigid_detectContactPairPerRigidPoint(
...
void HeightFieldRigidDetectionModule::contactDetection(PBDSolver* solver, Real dt)
{

    auto& allRigids = rigids;
    ...
    for (int i = 0; i < allRigids.size(); ++i)  //auto& prigid : allRigids)
    {
        ...
        // detect collision.
        float contactMu  = allRigids[i]->getMu();
        float threashold = rigidinfo.linearVelocity.norm() * dt;
        int   bdim       = 512;
        int   gdim       = cudaGridSize(vertices.size(), bdim);
        int   rigidid    = prigid->getId();
        hfrigid_detectContactPairPerRigidPoint<<<gdim, bdim>>>(m_contacts.begin() + m_nContacts,
                                                               nContactsi.begin(),
                                                               m_heightField,
                                                               vertices,
                                                               rigidinfo,
