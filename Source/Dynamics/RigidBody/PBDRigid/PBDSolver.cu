...
void PBDSolver::synFromBodiedToRigid()
{
    ...
    {
        cudaMemcpy(&(m_CPUBodys[0]), m_GPUBodys.begin(), sizeof(PBDBodyInfo<double>) * m_nPermanentBodies, cudaMemcpyDeviceToHost);
    }
...
void PBDSolver::setContactJoints(DeviceDArray<ContactInfo<double>>& contacts, int nContact)
{
    ...
    if (m_nJoints > 0 && m_GPUJoints.size() < m_nJoints)
    {
        ...
        {
            cudaMemcpy(m_GPUJoints.begin(), &(m_CPUJoints[0]), sizeof(PBDJoint<double>) * m_nPermanentJoints, cudaMemcpyHostToDevice);
        }
    }
    ...
    if (!m_useGPU)
    {
        ...
        cudaMemcpy(&(m_CPUJoints[0]), m_GPUJoints.begin(), sizeof(PBDJoint<double>) * m_nJoints, cudaMemcpyDeviceToHost);
    }
    _updateBodyPointerInJoint();
...
__global__ void initJoints(PBDJoint<double>* joints, int nJoints, PBDBodyInfo<double>* bodys)
{
    int tid = threadIdx.x + blockDim.x * blockIdx.x;
...
void PBDSolver::_updateBodyPointerInJoint()
{
    if (m_useGPU && m_nJoints > 0)
    {
        ...
        initJoints<<<griddim, blockdim>>>(m_GPUJoints.begin(), m_nJoints, m_GPUBodys.begin());
        cuSynchronize();
    }
    else
    {
...
void PBDSolver::_onRigidDataDirty()
{
    ...
    if (m_useGPU && m_nPermanentBodies > 0)
    {
        cudaMemcpy(m_GPUBodys.begin(), &(m_CPUBodys[0]), sizeof(PBDBodyInfo<double>) * m_nPermanentBodies, cudaMemcpyHostToDevice);
    }
}
                              ...
                              const Vector3d*            pos)
{
    ...
    pose.invRotate(n);
    w += n[0] * n[0] * pbody->invInertia[0] + n[1] * n[1] * pbody->invInertia[1] + n[2] * n[2] * pbody->invInertia[2];
...
__device__ double applyBodyPairCorrection(
    ...
    const PBDBodyInfo<double>* body0,
    const PBDBodyInfo<double>* body1,
    ...
    double                     maxlambda = -1.0)
{
    ...
    if (body0)
    {
        applyBodyCorrection(pose0, body0, n, bePositionCorr ? &joint.globalPose0.position : 0);
    }
    if (body1)
    {
        n *= -1;
        applyBodyCorrection(pose1, body1, n, bePositionCorr ? &joint.globalPose1.position : 0);
    }
    return lambda;
}
    ...
    double                     maxlambda = -1.0)
{
    ...
    if (body0)
    {
        applyVelocityCorrection(linv0, angv0, body0->pose, body0, n, bePositionCorr ? &joint.globalPose0.position : 0);
    }
    if (body1)
    {
        n *= -1;
        applyVelocityCorrection(linv1, angv1, body1->pose, body1, n, bePositionCorr ? &joint.globalPose1.position : 0);
    }
    return lambda;
}
...
__device__ double limitAngle(
    BodyPose<double>&       pose0,
    BodyPose<double>&       pose1,
    PBDBodyInfo<double>*    body0,
    PBDBodyInfo<double>*    body1,
    const PBDJoint<double>& joint,
    const Vector3d&         n,
    const Vector3d&         a,
    const Vector3d&         b,
    ...
    double                  maxCorr = CUDART_PI)
{
    ...
    if (phi < minAngle || phi > maxAngle)
    {
        ...
        if (phi > maxCorr)
            omega *= maxCorr / phi;
        return applyBodyPairCorrection(pose0, pose1, body0, body1, joint, omega, dt, false);
    }
    return 0;
}
...
__global__ void PBDForward(PBDBodyInfo<double>* bodys, int nBodys, PBDJoint<double>* joints, int nJoints, Vector3d* dx, Quaterniond* dq, int* nConstraint, int nStep, double dt, double ome)
{
    ...
    for (int i = 0; i < nStep; ++i)
    {
        ...
        if (tid < nJoints)
        {
            ...
            if ((joints + tid)->rotationXLimited)
            {
                ...
                limitAngle(pose0, pose1, (joints + tid)->body0, (joints + tid)->body1, *(joints + tid), n, a0, a1, (joints + tid)->minAngleX, (joints + tid)->maxAngleX, subdt);
            }
            ...
            if ((joints + tid)->rotationYLimited)
            {
                ...
                limitAngle(pose0, pose1, (joints + tid)->body0, (joints + tid)->body1, *(joints + tid), n, b0, b1, (joints + tid)->minAngleY, (joints + tid)->maxAngleY, subdt);
            }
            ...
            if ((joints + tid)->rotationZLimited)
            {
                ...
                //Vector3d b0 = getQuatAxis1((joints + tid)->globalPose0.rotation);
                //Vector3d b1 = getQuatAxis1((joints + tid)->globalPose1.rotation);
                Vector3d n = getQuatAxis1((joints + tid)->globalPose0.rotation).cross(getQuatAxis1((joints + tid)->globalPose1.rotation)).normalize();
                ...
                limitAngle(pose0, pose1, (joints + tid)->body0, (joints + tid)->body1, *(joints + tid), n, c0, c1, (joints + tid)->minAngleZ, (joints + tid)->maxAngleZ, subdt);
            }
            ...
            applyBodyPairCorrection(pose0, pose1, (joints + tid)->body0, (joints + tid)->body1, *(joints + tid), pcorr, subdt, true);
...
/**
	...
	*/
__global__ void PBD_solveJoints(PBDJoint<double>* joints, int nJoints, Vector3d* dx, Quaterniond* dq, int* nConstraint, int nbodys, double dt)
{
    int tid = threadIdx.x + blockIdx.x * blockDim.x;
    ...
    if (tid < nJoints && (joints + tid)->active)
    {
        ...
        if ((joints + tid)->rotationXLimited)
        {
            ...
            if (n.norm() > 0 && a1.norm() > 0)
            {
                n.normalize();
                a1.normalize();
                limitAngle(pose0, pose1, (joints + tid)->body0, (joints + tid)->body1, *(joints + tid), n, a0, a1, (joints + tid)->minAngleX, (joints + tid)->maxAngleX, dt);
            }
        }
        ...
        if ((joints + tid)->rotationYLimited)
        {
            ...
            limitAngle(pose0, pose1, (joints + tid)->body0, (joints + tid)->body1, *(joints + tid), n, b0, b1, (joints + tid)->minAngleY, (joints + tid)->maxAngleY, dt);
        }
        ...
        if ((joints + tid)->rotationZLimited)
        {
            ...
            limitAngle(pose0, pose1, (joints + tid)->body0, (joints + tid)->body1, *(joints + tid), n, c0, c1, (joints + tid)->minAngleZ, (joints + tid)->maxAngleZ, dt);
        }
        ...
        if (joint->positionLimited)
        {
            ...
            Vector3d pcorr = (joints + tid)->globalPose1.position - (joints + tid)->globalPose0.position;
            Vector3d tmpv  = (joints + tid)->beContact ? (joints + tid)->normal : -getQuatAxis1((joints + tid)->globalPose0.rotation);
                ...
                || ((joints + tid)->maxDistance >= 0 && dn > (joints + tid)->maxDistance))
            {
                ...
                lambda            = applyBodyPairCorrection(pose0, pose1, (joints + tid)->body0, (joints + tid)->body1, *(joints + tid), pcorr, dt, true);
            ...
            lambda = applyBodyPairCorrection(pose0, pose1, (joints + tid)->body0, (joints + tid)->body1, *(joints + tid), pcorr, dt, true, lambda);
...
/**
	* @brief Solve PBD joints.
	*/
__global__ void PBD_solveConstraintJoints(PBDJoint<double>* joints, int nJoints, Vector3d* dx, Quaterniond* dq, double dt)
{
    int tid = threadIdx.x + blockIdx.x * blockDim.x;
    ...
    if (tid < nJoints && (joints + tid)->active && !((joints + tid)->beContact))
    {
        ...
        if ((joints + tid)->rotationXLimited)
        {
            ...
            if (n.norm() > 0 && a1.norm() > 0 && a0.norm() > 0)
            {
                ...
                a1.normalize();
                a0.normalize();
                deb_lambda_x = limitAngle(pose0, pose1, (joints + tid)->body0, (joints + tid)->body1, *(joints + tid), n, a0, a1, (joints + tid)->minAngleX, (joints + tid)->maxAngleX, dt);
            }
        }
        ...
        if ((joints + tid)->rotationYLimited)
        {
            ...
            deb_lambda_y = limitAngle(pose0, pose1, (joints + tid)->body0, (joints + tid)->body1, *(joints + tid), n, b0, b1, (joints + tid)->minAngleY, (joints + tid)->maxAngleY, dt);
        }
        ...
        if ((joints + tid)->rotationZLimited)
        {
            ...
            if (n.norm() > EPSILON)
            {
                ...
                deb_lambda_z = limitAngle(pose0, pose1, (joints + tid)->body0, (joints + tid)->body1, *(joints + tid), n, c0, c1, (joints + tid)->minAngleZ, (joints + tid)->maxAngleZ, dt);
            }
        }
...
/**
	...
	*/
__global__ void PBD_solveContactJoints(PBDJoint<double>* joints, int nJoints, Vector3d* dx, Quaterniond* dq, double dt)
{
    int tid = threadIdx.x + blockIdx.x * blockDim.x;
...
/**
	...
	*/
__global__ void PBD_updatePosChangeAndVelocity(PBDBodyInfo<double>* bodys, int nbodys, Vector3d* dx, Quaterniond* dq, /*int* nConstraint,*/
                                               double dt,
                                               double ome,
                                               bool   updateVel        = true,
                                               bool   resetNConstraint = false)
{
    int tid = threadIdx.x + blockIdx.x * blockDim.x;
...
/**
	...
	*/
__global__ void PBD_solveVelocity(PBDJoint<double>* joints, int nJoints, Vector3d* dlinv, Vector3d* dangv, /*int* nConstraint,*/ int nbodys, double dt)
{
    int tid = threadIdx.x + blockIdx.x * blockDim.x;
    ...
    if (tid < nJoints && (joints + tid)->active)
    {
        ...
        applyPairVelocityCorrection(linv0, angv0, linv1, angv1, (joints + tid)->body0, (joints + tid)->body1, *(joints + tid), dv, dt, false);
        ...
        applyPairVelocityCorrection(linv0, angv0, linv1, angv1, (joints + tid)->body0, (joints + tid)->body1, *(joints + tid), dv, dt, true);
        ...
        if (joint->beContact)
        {
            ...
            applyPairVelocityCorrection(
                linv0, angv0, linv1, angv1, (joints + tid)->body0, (joints + tid)->body1, *(joints + tid), dv, dt, true);
        }
...
/**
	* @brief solve velocity constraints.
	*/
__global__ void PBD_solveCollisionVelocity(PBDJoint<double>* joints, int nJoints, Vector3d* dlinv, Vector3d* dangv, int nbodys, double dt)
{
    int tid = threadIdx.x + blockIdx.x * blockDim.x;
    if (tid >= nJoints || !((joints + tid)->active))
        return;
    ...
    if (joint->beContact && joint->relVn <= 0)
    {
        ...
        ev        = dv.dot(joint->normal) - (ev > 2.0 * dt * 9.8 ? ev : 0);
        dv        = joint->normal * ev;
        applyPairVelocityCorrection(linv0, angv0, linv1, angv1, joint->body0, joint->body1, *joint, dv, dt, true);
        ...
        dv = vt * ev;
        applyPairVelocityCorrection(
            linv0, angv0, linv1, angv1, joint->body0, joint->body1, *joint, dv, dt, true);
    }
...
__global__ void PBD_solveCollisionPose(PBDJoint<double>* joints, int nJoints, Vector3d* dx, Quaterniond* dq, int* nConstraint, int nbodys, double dt)
{
    int tid = threadIdx.x + blockIdx.x * blockDim.x;
    ...
    if (tid < nJoints && (joints + tid)->active && (joints + tid)->beContact)
    {
        ...
        joint->normLambda = applyBodyPairCorrection(pose0, pose1, (joints + tid)->body0, (joints + tid)->body1, *(joints + tid), pcorr, dt, true);
...
__global__ void PBD_omegaCorrection(PBDJoint<double>* joints, int nJoints, Vector3d* dx, Quaterniond* dq, double* omega, int* nConstraint, int nbodys, double dt)
{
    int tid = threadIdx.x + blockIdx.x * blockDim.x;
    if (tid < nJoints && (joints + tid)->active && (joints + tid)->beContact)
    {
    ...
    for (int i = 0; i < m_numSubstep; ++i)
    {
        ...
        if (m_nBodies > 0)
        {
            ...
            PBD_integrateAndPrepareBody<<<griddim_body, blockdim_body>>>(m_GPUBodys.begin(),
                                                                         m_GPUPosChange.begin(),
                                                                         m_GPURotChange.begin(),
                                                                         m_GPULinvChange.begin(),
                                                                         m_GPUAngvChange.begin(),
                                                                         m_GPUConstraintCount.begin(),
                                                                         m_nBodies,
                                                                         subdt);
            //cudaDeviceSynchronize();
        ...
        if (m_nJoints > 0)
        {
            ...
            PBD_solveJoints<<<griddim_joint, blockdim_joint>>>(m_GPUJoints.begin(), m_nJoints, m_GPUPosChange.begin(), m_GPURotChange.begin(), m_GPUConstraintCount.begin(), m_nBodies, subdt);
            //cudaDeviceSynchronize();
            //err1 = cudaGetLastError();
            cuSynchronize();
        }
        ...
        if (m_nBodies > 0)
        {
            PBD_updatePosChangeAndVelocity<<<griddim_body, blockdim_body>>>(m_GPUBodys.begin(), m_nBodies, m_GPUPosChange.begin(), m_GPURotChange.begin(), /* m_GPUConstraintCount.begin(),*/
                                                                            subdt,
                                                                            1.0);
            cuSynchronize();
        }
...
void PBDSolver::integrateBodyForce(Real dt)
{
    if (m_nBodies > 0)
    {
        cuExecute(m_nBodies, PBD_integrateForce, m_GPUBodys.begin(), m_nBodies, dt);
    }
}
...
void PBDSolver::integrateBodyVelocity(Real dt)
{
    if (m_nBodies > 0)
    {
        cuExecute(m_nBodies, PBD_integrateVelocity, m_GPUBodys.begin(), m_GPUPosChange.begin(), m_GPURotChange.begin(), m_GPULinvChange.begin(), m_GPUAngvChange.begin(), m_GPUConstraintCount.begin(), m_nBodies, dt);
    }
}
...
void PBDSolver::solveSubStepGPU(Real dt)
{
    ...
    if (m_nJoints > 0)
    {
        ...
        PBD_solveJoints<<<griddim_joint, blockdim_joint>>>(m_GPUJoints.begin(), m_nJoints, m_GPUPosChange.begin(), m_GPURotChange.begin(), m_GPUConstraintCount.begin(), m_nBodies, dt);
    ...
    if (m_nJoints > 0)
    {
        ...
        PBD_solveVelocity<<<griddim_joint, blockdim_joint>>>(m_GPUJoints.begin(), m_nJoints, m_GPULinvChange.begin(), m_GPUAngvChange.begin(), /*m_GPUConstraintCount.begin(),*/
                                                             m_nBodies,
                                                             dt);
        cudaDeviceSynchronize();
        cuSynchronize();
        ...
        PBD_updateVelocityChange<<<griddim_body, blockdim_body>>>(m_GPUBodys.begin(),
                                                                  m_GPULinvChange.begin(),
                                                                  m_GPUAngvChange.begin(), /*m_GPUConstraintCount.begin(),*/
                                                                  m_nBodies,
                                                                  dt,
                                                                  1.0);
        cuSynchronize();
    ...
    if (/*m_nPermanentJoints*/ m_nJoints > 0)
    {
        ...
        for (int iteri = 0; iteri < m_numContactSolveIter; ++iteri)
        {
            ...
            if (m_nPermanentJoints > 0)
            {
                ...
                PBD_solveConstraintJoints<<<griddim_joint, blockdim_joint>>>(m_GPUJoints.begin(), m_nPermanentJoints, m_GPUPosChange.begin(), m_GPURotChange.begin(), dt);
                cuSynchronize();
            }
            ...
            if (/*iteri >= m_numContactSolveIter -1&&*/ m_nJoints > m_nPermanentJoints)
            {
                ...
                PBD_solveContactJoints<<<griddim_joint, blockdim_joint>>>(
                    m_GPUJoints.begin() + m_nPermanentJoints, m_nJoints - m_nPermanentJoints, m_GPUPosChange.begin(), m_GPURotChange.begin(), dt);
                cuSynchronize();
            }
            ...
            PBD_updatePosChange<<<griddim_body, blockdim_body>>>(m_GPUBodys.begin(),
                                                                 m_GPUPosChange.begin(),
                                                                 m_GPURotChange.begin(), /*m_GPUConstraintCount.begin(),*/
                                                                 m_nBodies,
                                                                 dt,
                                                                 1.0,
    ...
    double            dt)
{
    ...
    applyBodyPairCorrection(pose0, pose1, (joints + tid)->body0, (joints + tid)->body1, *(joints + tid), pcorr, dt, true, lambda);
...
__global__ void PBD_PBC_updatePosChangeAndVelocityChange(PBDBodyInfo<double>* bodys, int nbodys, Vector3d* dx, Quaterniond* dq, /*int* nConstraint,*/
                                                         double dt,
                                                         double ome)
{
    int tid = threadIdx.x + blockIdx.x * blockDim.x;
...
/**
	* @brief solve velocity constraints.
	*/
__global__ void PBD_PBC_solveCollisionVelocity(PBDJoint<double>* joints, int nJoints, Vector3d* parVel, Vector3d* dlinv, Vector3d* dangv, int nbodys, double dt)
{
    int tid = threadIdx.x + blockIdx.x * blockDim.x;
    if (tid >= nJoints || !((joints + tid)->active))
        return;
...
void PBDParticleBodyContactSolver::forwardSubStep(Real dt, bool updateVel)
{
    ...
    // Solve contact.
    cuExecute(m_joints->size(), PBD_PBC_solveParticleBodyContact, m_joints->begin(), m_joints->size(), m_bodyPosChange.begin(), m_bodyRotChange.begin(), dt);
    ...
    cuExecute(m_body->size(), PBD_updatePosChangeAndVelocity, m_body->begin(), m_body->size(), m_bodyPosChange.begin(), m_bodyRotChange.begin(), dt, m_omega, updateVel);
    ...
    if (false)
    {
        ...
        cuExecute(m_particleVel->size(), PBD_PBC_updateParticleVelocityChange, m_particleVel->begin(), m_particle->begin(), m_particleVel->size(), dt, m_omega);
    }
...
void PBDParticleBodyContactSolver::solveCollisionVelocity(Real dt)
{
    ...
    cuExecute(m_joints->size(), PBD_PBC_solveCollisionVelocity, m_joints->begin(), m_joints->size(), m_particleVel->begin(), m_bodyLinvChange.begin(), m_bodyAngvChange.begin(), m_body->size(), dt);

    cuExecute(m_body->size(), PBD_updateVelocityChange, m_body->begin(), m_bodyLinvChange.begin(), m_bodyAngvChange.begin(), m_body->size(), dt, m_omega);
}
...
bool PBDParticleBodyContactSolver::buildJoints()
{
    ...
    cuExecute(m_contacts->size(), PBD_PBC_buildContactJoint, m_joints->begin(), *m_contacts, m_body->begin(), m_particle->begin());
}

void PBDParticleBodyContactSolver::updateParticleBodyInfo(double mu)
{
    if (!m_particlePos || !m_particle || m_particlePos->size() <= 0)
        return;

    if (m_particle->size() != m_particlePos->size())
        m_particle->resize(m_particlePos->size());

    cuExecute(m_particle->size(), PBD_PBC_updateParticleBodyInfo, m_particle->begin(), m_particlePos->begin(), m_particleVel->begin(), m_particleMass->begin(), m_particlePos->size(), mu);
}
    ...
    double                 dt)
{
    ...
    for (int ne = 0; ne < nbSize; ne++)
    {
        ...
        if (r > EPSILON)
        {
            ...
            Vector3d dp_ij = /*10.0f**/ (pos_i - posArr[j]) * ((lamda_i * massj / rho_i_ + lambdas[j] * massi / rhoTar[j]) * kern.Gradient(r, smoothingLength) * (1.0 / r));
            dpi += dp_ij;
        }
    }
...
{
    if (!m_particlePos || !m_particleVel || !m_particleRho2D || m_neighbor.isEmpty())
        return;
    ...
    cuExecute(nsize, PBD_DS_computeTargetRho, m_targetRho, *m_particleRho2D, *m_particleVel, m_rho0, minh, dt);
    ...
    for (int i = 0; i < 1; ++i)
    {
        ...
        cuExecute(nsize, PBD_DS_computeLambdas, m_lambda, *m_particleRho2D, m_targetRho, *m_particlePos, *m_particleMass, m_neighbor.getValue(), m_kernel, m_smoothLength, dt, 0.01);

        // Compute dx.
        if (m_particleDpos.size() != nsize)
            m_particleDpos.resize(nsize);
        cuExecute(nsize, PBD_DS_computeDisplacement, m_particleDpos, m_lambda, m_targetRho, *m_particlePos, *m_particleMass, m_neighbor.getValue(), m_kernel, m_smoothLength, dt);
        ...
        cuExecute(nsize, PBD_DS_updateParticleVelocityChange, m_particleVel->begin(), m_particleDpos.begin(), nsize, dt);
    }
