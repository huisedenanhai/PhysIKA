...
COMM_FUNC Real TPoint3D<Real>::areaTriangle(const TTriangle3D<Real>& triangle, const Real& r) const
{
    ...
    if (
        ((triangle.v[0] - p2.origin).norm() < r2 && (triangle.v[1] - p2.origin).norm() < r2 && (triangle.v[2] - p2.origin).norm() > r2) || ((triangle.v[0] - p2.origin).norm() < r2 && (triangle.v[1] - p2.origin).norm() > r2 && (triangle.v[2] - p2.origin).norm() < r2) || ((triangle.v[0] - p2.origin).norm() > r2 && (triangle.v[1] - p2.origin).norm() < r2 && (triangle.v[2] - p2.origin).norm() < r2))
    {
        ...
        Coord3D ps1 = p2.project(s1).origin + (outp.origin - p2.project(s1).origin) / (outp.origin - p2.project(s1).origin).norm() * glm::sqrt(r2 * r2 - (p2.project(s1).origin - p2.origin).norm() * (p2.project(s1).origin - p2.origin).norm());

        Coord3D ps2 = p2.project(s2).origin + (outp.origin - p2.project(s2).origin) / (outp.origin - p2.project(s2).origin).norm() * glm::sqrt(r2 * r2 - (p2.project(s2).origin - p2.origin).norm() * (p2.project(s2).origin - p2.origin).norm());
        ...
        if (ret < 0)
            return Real(0);
        if (!(ret < triangle.area()))
            ret = triangle.area();
        if ((!(ret < Real(M_PI) * r2 * r2)))
            return Real(M_PI) * r2 * r2;
        return ret;
    }
        ...
        ((triangle.v[0] - p2.origin).norm() > r2 && (triangle.v[1] - p2.origin).norm() > r2 && (triangle.v[2] - p2.origin).norm() < r2) || ((triangle.v[0] - p2.origin).norm() > r2 && (triangle.v[1] - p2.origin).norm() < r2 && (triangle.v[2] - p2.origin).norm() > r2) || ((triangle.v[0] - p2.origin).norm() < r2 && (triangle.v[1] - p2.origin).norm() > r2 && (triangle.v[2] - p2.origin).norm() > r2))
    {
        ...
        Coord3D ps1 = p2.project(s1).origin + (oup1.origin - p2.project(s1).origin) / (oup1.origin - p2.project(s1).origin).norm() * glm::sqrt(r2 * r2 - (p2.project(s1).origin - p2.origin).norm() * (p2.project(s1).origin - p2.origin).norm());

        if ((inp.origin - p2.project(s1).origin).norm() < EPSILON)
            ps1 = inp.origin;

        Coord3D ps2 = p2.project(s2).origin + (oup2.origin - p2.project(s2).origin) / (oup2.origin - p2.project(s2).origin).norm() * glm::sqrt(r2 * r2 - (p2.project(s2).origin - p2.origin).norm() * (p2.project(s2).origin - p2.origin).norm());

        if ((inp.origin - p2.project(s2).origin).norm() < EPSILON)
            ps2 = inp.origin;
        ...
        if (distance(so) < r - EPSILON)
        {
            ...
            Coord3D       plo1 = p2.project(lo).origin + (oup1.origin - p2.project(lo).origin) / (oup1.origin - p2.project(lo).origin).norm() * glm::sqrt(r2 * r2 - (p2.project(lo).origin - p2.origin).norm() * (p2.project(lo).origin - p2.origin).norm());
            //if ((inp.origin - p2.project(s1).origin).norm() < EPSILON)plo1 = inp.origin;
            Coord3D plo2 = p2.project(lo).origin + (oup2.origin - p2.project(lo).origin) / (oup2.origin - p2.project(lo).origin).norm() * glm::sqrt(r2 * r2 - (p2.project(lo).origin - p2.origin).norm() * (p2.project(lo).origin - p2.origin).norm());
    ...
    for (int j = 0; j < 3; j++)
    {
        ...
        if ((distance(so)) < r - EPSILON)
        {
            ...
            Real tmp1 = (((plo1 + plo2) / 2.0 - p2.origin) / ((plo1 + plo2) / 2.0 - p2.origin).norm()).dot((plo1 + plo2) / 2.0 - oup3.origin) / ((plo1 + plo2) / 2.0 - p2.origin).norm();
...
COMM_FUNC TSegment3D<Real> TLine3D<Real>::proximity(const TAlignedBox3D<Real>& box) const
{
    ...
    auto face = [](Coord3D& pnt, Real& final_t, const Coord3D& dir, const Coord3D& PmE, const Coord3D& boxExtent, int i0, int i1, int i2) {
        Coord3D PpE;
...
COMM_FUNC int TLine3D<Real>::intersect(const TAlignedBox3D<Real>& abox, TSegment3D<Real>& interSeg) const
{
    ...
    if (clip(+lineDir[0], -offset[0] - boxExtent[0], t0, t1) && clip(-lineDir[0], +offset[0] - boxExtent[0], t0, t1) && clip(+lineDir[1], -offset[1] - boxExtent[1], t0, t1) && clip(-lineDir[1], +offset[1] - boxExtent[1], t0, t1) && clip(+lineDir[2], -offset[2] - boxExtent[2], t0, t1) && clip(-lineDir[2], +offset[2] - boxExtent[2], t0, t1))
    {
        if (t1 > t0)
        {
...
COMM_FUNC bool TAlignedBox3D<Real>::meshInsert(const TTriangle3D<Real>& tri) const
{
    ...
    Coord3D        seg[12][2] = {
        P0, P1, P0, P2, P0, P4, P7, P6, P7, P5, P7, P3, P2, P3, P2, P6, P3, P1, P6, P4, P4, P5, P1, P5
    };

    Coord3D m_triangle_index[12][3] = {
        P0, P1, P2, P0, P1, P4, P0, P2, P4, P7, P5, P6, P7, P3, P5, P7, P3, P6, P2, P4, P6, P1, P4, P5, P1, P2, P3, P3, P5, P1, P5, P6, P4, P3, P6, P2
    };
...
COMM_FUNC bool TOrientedBox3D<Real>::point_intersect(const TOrientedBox3D<Real>& OBB, Coord3D& interNorm, Real& interDist, Coord3D& p1, Coord3D& p2) const
{
    ...
    Segment3D s[12];
    s[0]  = Segment3D(p[0].origin, p[1].origin);
    s[1]  = Segment3D(p[0].origin, p[2].origin);
    s[2]  = Segment3D(p[0].origin, p[4].origin);
    s[3]  = Segment3D(p[3].origin, p[1].origin);
    s[4]  = Segment3D(p[3].origin, p[2].origin);
    s[5]  = Segment3D(p[3].origin, p[7].origin);
    s[6]  = Segment3D(p[6].origin, p[7].origin);
    s[7]  = Segment3D(p[6].origin, p[2].origin);
    s[8]  = Segment3D(p[6].origin, p[4].origin);
    s[9]  = Segment3D(p[5].origin, p[7].origin);
    s[10] = Segment3D(p[5].origin, p[1].origin);
    s[11] = Segment3D(p[5].origin, p[4].origin);
    ...
    if (max_u < -OBB.extent[0] || min_u > OBB.extent[0])
        return false;
    ...
    if (max_v < -OBB.extent[1] || min_v > OBB.extent[1])
        return false;
    ...
    if (max_w < -OBB.extent[2] || min_w > OBB.extent[2])
    {
        return false;
    }
...
template <typename Real>
COMM_FUNC Real TAlignedBox3D<Real>::width() const
{
    return v1[0] - v0[0];
}
template <typename Real>
COMM_FUNC Real TAlignedBox3D<Real>::height() const
{
    return v1[1] - v0[1];
}
template <typename Real>
COMM_FUNC Real TAlignedBox3D<Real>::depth() const
{
    return v1[2] - v0[2];
}
}  // namespace PhysIKA
